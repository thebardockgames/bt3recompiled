...
# MSYS2/MinGW's CMake sometimes fails to populate CMAKE_SYSTEM_PROCESSOR when
# using the Ninja generator, which trips Dolphin's architecture detection.
# Force it on Windows so the configure step doesn't need a manual override.
ifeq ($(OS),Windows_NT)
MODERNGEKKO_CMAKE_EXTRA ?= -DCMAKE_SYSTEM_PROCESSOR=AMD64
endif
...
moderngekko:
	cmake -S $(MODERNGEKKO_DIR) -B $(MODERNGEKKO_BUILD) -G Ninja -DCMAKE_BUILD_TYPE=$(CMAKE_BUILD_TYPE) $(MODERNGEKKO_CMAKE_EXTRA)
